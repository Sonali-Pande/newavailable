<?php
	...
	function ShowListQuestion($quest_id){
		if(!empty($GLOBALS['json_data'])){
			...
			$output.='<div class="btn-group mt-n5 mx-2" role="group" aria-label="List Category" id="myBtnContainer">
				<button type="button" class="btn btn_active btn-light border p-2" id="all_btn" onclick="filterSelection(`all`,`list`)">All</button>
				<button type="button" class="btn btn-outline-primary" id="attempt_btn" onclick="filterSelection(`attempt`,`list`)" >Attempt</button>
				<button type="button" class="btn btn-outline-warning" id="wrong_btn" onclick="filterSelection(`wrong`,`list`)">Unattempt</button>
			</div>';
	...
	//get The Previous Question  or Next Question
